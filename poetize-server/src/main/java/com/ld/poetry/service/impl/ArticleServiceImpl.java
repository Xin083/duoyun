...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
        ...
        try {
            if (articleVO.getViewStatus()) {
                ...
                if (!CollectionUtils.isEmpty(emails)) {
                    ...
                    mailUtil.sendMailMessage(emails, "您有一封来自" + (webInfo == null ? "maoye" : webInfo.getWebName()) + "的回执！", text);
                }
            }
        } catch (Exception e) {
    ...
    private String getSubscribeMail(String labelName, String articleTitle) {
        WebInfo webInfo = (WebInfo) PoetryCache.get(CommonConst.WEB_INFO);
        String webName = (webInfo == null ? "maoye" : webInfo.getWebName());
     ...
     * 点赞
     **/
    public PoetryResult<String> likeArticle(Integer id) {
        // 根据id获取文章
...
//            return PoetryResult.fail("已经点赞过了，不需要再次点赞！");
//        }
        // 更新点赞数
        article.setLikeCount(article.getLikeCount() + 1);
        articleMapper.updateById(article);

        return PoetryResult.success("点赞成功");
    }
