...
public class AdminController {
    ...
    @LoginCheck(2)
    public PoetryResult changePassword(@RequestBody Password password) {
        LambdaQueryWrapper<User> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(User::getId, PoetryUtil.getUserId());
        User user = userService.getOne(wrapper);
        //如果原密码加密后跟数据库中的密码一致
        if (SecureUtil.md5(password.getOldPassword()).equals(user.getPassword())){
            user.setPassword(SecureUtil.md5(password.getNewPassword()));
            userService.updateById(user);
            return PoetryResult.success("修改成功,请重新登录!");
        }else if (!password.getNewPassword().equals(password.getNewPassword2())) {
            return PoetryResult.success("两次密码不一致！");
        }else {
            return PoetryResult.success("原密码错误!");
        }
    }
    @PostMapping("/user/updateUserInfo")
    @LoginCheck(1)
    public PoetryResult updateUserInfo(@RequestBody User user) {
        if (user.getId() != null) {
            userService.updateById(user);
        }else {
            return PoetryResult.fail("用户id不能为空！");
        }
        return PoetryResult.success();
    }
    ...
    @PostMapping("/upload")
    @LoginCheck(1)
    public PoetryResult upload(@RequestBody JSONObject jsonObject){
        //接受上传的文件
